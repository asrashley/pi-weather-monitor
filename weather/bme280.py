...
class Bme280Probe:
  def __init__(self, bus_id: int = 1, i2c_addr: int = 0x77):
    # Rev 1 Pi uses bus 0
  ...
  def read_id(self):
    ...
    REG_ID     = 0xD0
    (chip_id, chip_version) = self.bus.read_i2c_block_data(self.i2c_addr, REG_ID, 2)
  ...
  def read_values(self):
    ...
    OVERSAMPLE_HUM = 2
    self.bus.write_byte_data(self.i2c_addr, REG_CONTROL_HUM, OVERSAMPLE_HUM)

    control = OVERSAMPLE_TEMP<<5 | OVERSAMPLE_PRES<<2 | MODE
    self.bus.write_byte_data(self.i2c_addr, REG_CONTROL, control)

    # Read blocks of calibration data from EEPROM
    # See Page 22 data sheet
    cal1 = self.bus.read_i2c_block_data(self.i2c_addr, 0x88, 24)
    cal2 = self.bus.read_i2c_block_data(self.i2c_addr, 0xA1, 1)
    cal3 = self.bus.read_i2c_block_data(self.i2c_addr, 0xE1, 7)
    ...
    time.sleep(wait_time / 1000.0)  # Wait the required time

    # Read temperature/pressure/humidity
    data = self.bus.read_i2c_block_data(self.i2c_addr, REG_DATA, 8)
  ...
  def main():
    probe = Bme280Probe()
    (chip_id, chip_version) = probe.read_id()
    print("Chip ID     :", chip_id)
    print("Version     :", chip_version)

    temperature,pressure,humidity = probe.read_values()
